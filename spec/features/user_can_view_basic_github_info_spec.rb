...
feature "user visits dashboard" do
  it "displays github profile info" do
    user = User.create(oauth_token: ENV['GITHUB_ACCESS_TOKEN'])
    allow_any_instance_of(ApplicationController).to receive(:current_user).and_return(user)

    visit dashboard_index_path

    expect(page).to have_content("Andy Dymond")
    expect(page).to have_content("andymond")
    expect(page).to have_content("Denver, CO")
    expect(page).to have_content("andy.dymond@gmail.com")
